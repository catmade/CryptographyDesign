using CryptographyDesign.CipherKey;
...
{
    public class MyCipher : CipherAbstract, ICipher<string, string>
    {
        /// <summary>
        /// 符号集
        /// </summary>
        ...
        public MyCipher(int[] vigenereKey, int affineKeyA, int affineKeyB, int[,] hillMatrix): base("组合加密")
        {
        ...
        public bool IsPlainSuitable(string plain, out string message, out string sender)
        {
            sender = name;
            message = "suitable";
            ...
            {
                bool isLowercase = item >= 0 && item <= 25;
                if (!isLowercase)
                {
                    message = "明文必须是小写字母";
                    return false;
        ...
        public bool IsCipherSuitable(string cipher, out string message, out string sender)
        {
            sender = name;
            message = "suitable";
